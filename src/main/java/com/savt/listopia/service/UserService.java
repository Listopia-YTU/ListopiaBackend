...
import com.savt.listopia.exception.UserException;
...
public class UserService {
    ...
    public UserService(UserRepository userRepository, ModelMapper modelMapper, PrivateMessageRepository privateMessageRepository, MovieRepository movieRepository, MovieCommentRepository movieCommentRepository) {
    ...
    public User getUserByEmailPassword(String email, String plainPassword) {
        User user = userRepository.findByEmail(email).orElseThrow( () -> new UserException("mail_does_not_exists") );
        // @todo: handle time based mail enum
        if (verifyUserPassword(user, plainPassword))
            return user;

        throw new UserException("mail_does_not_exists");
    }
    ...
    public Page<MovieCommentDTO> getMovieCommentIsReported(Boolean isReported, int page, int size) {
        Pageable pageable = PageRequest.of(page, size, Sort.by("sentAtTimestampSeconds").descending());
